...
def addGrille(ds):
	...
	ds['hc'] = gd.hc
	ds['h'] = gd.h
	ds['Vtransform'] = gd.Vtransform  #ligne à commenter pour GIGATL3
	ds['sc_r'] = gd.sc_r
	ds['sc_w'] = gd.sc_w
	ds['Cs_r'] = gd.Cs_r
	ds['Cs_w'] = gd.Cs_w
...
def get_z(run, zeta=None, h=None, vgrid='r', hgrid='r', vtrans=None):
    ...
    sc=ds['sc_'+vgrid]
    cs=ds['Cs_'+vgrid]
    # vtrans = 2 # ligne a décommenter pour GIGATL3
    if vtrans == 2:
...
def _rtree(v, xold, yold, xnew, ynew):
    ...
    mesh.packing(np.vstack((xold.flatten(), yold.flatten())).T,v.flatten())

    # Interpolate the grid
    idw, neighbors = mesh.inverse_distance_weighting(
                                    np.vstack((xnew.flatten(), ynew.flatten())).T,
                                    within=True,
                                    k=4)
    
    return np.reshape(idw,xnew.shape)
    
def rtree_xr(ds, var, xnew1d, ynew1d):

    #[N, M, L] = var.shape
    #kwargs = {"longitude":longitude, "latitude":latitude, "depth":depth}
    ...
    # Find horizontal coordinates of the variable 
    x = ([var.coords[s] for s in var.coords if "xi_" in s][0]).astype('float32')
    y = ([var.coords[s] for s in var.coords if "eta_" in s][0]).astype('float32')

    # Adapt the mask on the C-grid
    mask2d = ds.mask_rho
    if 'u' in xdim : mask2d = rho2u(mask2d,ds)
    if 'v' in ydim : mask2d = rho2v(mask2d,ds)        
    
    # Transform vectors in 3D grids
    xnew, ynew= np.meshgrid(xnew1d, ynew1d)

    
    # Fill masked values with nan
#     var = var.where(mask2d==1)
    
    slice = xr.apply_ufunc(_rtree, var.values, x.values, y.values,
                          xnew, ynew,
                          dask='parallelized', output_dtypes=[np.float64]).astype('float32')
        
    return xr.DataArray(slice, dims=[ydim,xdim],
                        coords={xdim: xnew1d, ydim:ynew1d}).astype('float32')
